...
- ✅ **weight-sync NCCL hang 已解决（含 2 引擎）**：`ROLLOUT_NUM_GPUS=1` 单引擎 2.0-2.2s 完成；**2 引擎（`ROLLOUT_NUM_GPUS=2`，world_size=3）实测也正常**——两个引擎都 init_weights_update_group（rank 1/2）、NCCL group 建齐、update_weights 2.3s 完成、GPU2/3 并行 rollout。§9 记录的「只有 1 引擎 init」的 hang 是 **libcudart/attention 修复之前**的副作用（train actor 在 weight update 阶段因别的问题卡死），全套修复就位后不复现。2 引擎用于加速 claude_code 慢 rollout。
